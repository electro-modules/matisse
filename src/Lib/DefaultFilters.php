...
class DefaultFilters
{
  ...
  function filter_call ($instance, $method, ...$args)
  {
    if (is_object ($instance)) {
      if (is_object ($instance) && method_exists ($instance, $method)) return $instance->$method (...$args);
  ...
  /**
   * Accesses a dynamic index of an array or a dynamic property of an object.
   *
   * @param mixed          $v
   * @param string|integer $k The index value or the property name.
   * @param mixed          $default
   * @return mixed
   */
  function filter_at ($v, $k, $default = '')
  {
    return getField ($v, $k, $default);
  }
  ...
  {
    return map ($v, function ($e) use ($field) {
  ...
  {
    if (function_exists ('inspect')) inspect ($v);
    return '';
  }
